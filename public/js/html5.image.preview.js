...
function previewImage(el,widths,limit){
	...
	if (!device.android){ // Since android doesn't handle file types right, do not do this check for phones
		if (!file.type.match(imageType)) {
			...
			output.innerHTML='';
			description.innerHTML='Imagem invalida';
	...
	reader.onload = (function(aImg) {
		return function(e) {
			...
			var description = document.createElement('p');
			description.innerHTML = '<br />Imagem <b>' + format + '</b> Tamanho<b>'+(e.total/1024).toFixed(2)+'</b> KB.';
			...
			if (allowedTypes.indexOf(format)>=0 && e.total<(limit*1024*1024)){
				for (var size in widths){
					...
					image.title = 'Pré Vizualizacao '+widths[size]+'px';
					output.appendChild(image);
				}

				description.innerHTML += '<br /><span style="color:green;">Upload Concluído</span>';
			} else {
				description.innerHTML += '<br /><span style="color:red;">Qual é o formato / tamanho errado! Formatos aceitos:' + allowedTypes.join(', ') + '. O limite de tamanho é: '+limit+'MB</span>';
			}						
